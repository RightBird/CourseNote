...
定义**索引(index)**为一个TM $M$ 的编码 "M" ，同时 "M" 也是 R.E. set $L(M)$ 的索引。

定义**索引集(index set)** 是一个 R.E. sets 的 collection 的索引集合，即如果 $C$ 是一个R.E. sets的集合，则 $F(C)=\{^"M^":L(M)\in C\}$
...
![](pic/5-14.png)![](pic/5-15.png)![](pic/5-16.png)
